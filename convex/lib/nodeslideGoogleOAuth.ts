...
export interface NodeSlideGoogleOAuthConfig {
  clientId: string;
  clientSecret: string;
...
export function resolveNodeSlideGoogleOAuthConfig(input: {
  clientId?: string | undefined;
  clientSecret?: string | undefined;
  encryptionKey?: string | undefined;
  redirectUri?: string | undefined;
  allowedOrigins?: string | undefined;
}): NodeSlideGoogleOAuthConfig {
  ...
  let origins: string[];
  try {
    origins = allowedNodeSlideOrigins(input.allowedOrigins);
  } catch {
    throw new Error('Google Slides connection is not configured for this deployment.');
  }
  if (!clientId || !clientSecret || !encryptionKey || !redirectUri || !origins.length) {
    throw new Error('Google Slides connection is not configured for this deployment.');
  }
  try {
    if (base64UrlToBytes(encryptionKey).byteLength !== 32) throw new Error('invalid key');
    const redirect = new URL(redirectUri);
    if (redirect.protocol !== 'https:' && !isLoopbackHostname(redirect.hostname)) {
      throw new Error('unsafe redirect');
    }
    if (redirect.username || redirect.password || redirect.hash) throw new Error('unsafe redirect');
    return {
      clientId,
      clientSecret,
      encryptionKey,
...
/**
 * The origins an OAuth `returnTo` is allowed to name. The fallback is this
 * app's own production origin: an unset `NODESLIDE_APP_ORIGINS` must not leave
 * the allowlist pointing at a different deployment, because the allowlist is
 * the only thing stopping the callback from redirecting somewhere else.
 */
export const NODESLIDE_DEFAULT_APP_ORIGIN = 'https://nodeslide.vercel.app';

export function allowedNodeSlideOrigins(value: string | undefined): string[] {
  const configured = (value ?? NODESLIDE_DEFAULT_APP_ORIGIN)
    .split(',')
