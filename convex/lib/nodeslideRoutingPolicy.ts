...
/**
 * This module hosts TWO independent NodeSlide routing policies. They share a
 * problem domain but not a decision, and neither supersedes the other:
 *
 *  1. `nodeslide.model-routing/v1` — the shipped orchestrator/worker split
 *     (`resolveNodeSlideRouting`). Decides whether bulk copy work is delegated
 *     to a cheap advisory executor. Consumed by `nodeslideEditPlanner.ts`.
 *  2. `nodeslide.auto-routing/v1` — provider/model selection with consent,
 *     capability, context, availability and cost-cap gates
 *     (`decideNodeSlideAutoRoute`). Consumed by `nodeslideRoutingReceipt.ts`.
 *
 * Policy (1) below is unchanged and remains the meaning of the unprefixed
 * `NODESLIDE_ROUTING_POLICY_VERSION` / `NodeSlideRoutingDecision` names, because
 * `nodeslideEditPlanner.ts` writes that literal into a shipped receipt. Policy
 * (2)'s equivalents are namespaced `..._AUTO_ROUTING_...` / `NodeSlideAutoRouting...`.
 *
 * NOTE: policy (1) was previously dependency-free by design so it could be
 * lifted into a product-neutral `model-routing.v1` contract. Policy (2) imports
 * the pi-ai OpenRouter catalog and `shared/nodeslide`, and builds its catalog at
 * module scope, so that property no longer holds for this file. Policy (1)'s own
 * code is still import-free and can be extracted unchanged if that is wanted.
 */

// ---------------------------------------------------------------------------
// Policy 1 — nodeslide.model-routing/v1 (orchestrator/worker split, Track B2)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Policy 2 — nodeslide.auto-routing/v1 (provider/model selection + cost gates)
// ---------------------------------------------------------------------------

export const NODESLIDE_AUTO_ROUTING_POLICY_VERSION = 'nodeslide.auto-routing/v1' as const;
...
interface NodeSlideAutoRoutingDecisionBase {
  policyVersion: typeof NODESLIDE_AUTO_ROUTING_POLICY_VERSION;
  providerNativeEffort: NodeSlideReasoningEffort | null;
  reasons: readonly string[];
  fallbackReceipt: NodeSlideFallbackReceipt;
}

export type NodeSlideAutoRoutingDecision =
  | (NodeSlideAutoRoutingDecisionBase & {
      kind: 'selected';
      route: NodeSlideSelectedRoute;
...
function selectedDecision(
  ...
  return {
    kind: 'selected',
    policyVersion: NODESLIDE_AUTO_ROUTING_POLICY_VERSION,
...
function decideDeterministicOnly(input: NodeSlideRoutingPolicyInput): NodeSlideAutoRoutingDecision {
