...
describe('NodeSlide durable budget cost ledger', () => {
  ...
  it('distinguishes a cap breach from an overflow and from a malformed quote', () => {
    const reserved = reserveNodeSlideBudgetCost(
      { capMicroUsd: 100, actualMicroUsd: 0, reservedMicroUsd: 0, unreconciledMicroUsd: 0 },
      60,
    );
    let thrown: unknown;
    try {
      reserveNodeSlideBudgetCost(reserved, 41);
    } catch (error) {
      thrown = error;
    }
    expect(thrown).toBeInstanceOf(NodeSlideBudgetLedgerError);
    expect((thrown as NodeSlideBudgetLedgerError).code).toBe('budget_exceeded');

    // A quote large enough to leave safe-integer range is reported as an
    // arithmetic overflow, NOT as a cap breach: the sum is never computed, so
    // the ledger cannot honestly say the cap was the thing that was exceeded.
    let overflow: unknown;
    try {
      reserveNodeSlideBudgetCost(reserved, Number.MAX_SAFE_INTEGER);
    } catch (error) {
      overflow = error;
    }
    expect((overflow as NodeSlideBudgetLedgerError).code).toBe('quote_overflow');

    let negative: unknown;
    try {
      reserveNodeSlideBudgetCost(reserved, -1);
    } catch (error) {
      negative = error;
    }
    expect((negative as NodeSlideBudgetLedgerError).code).toBe('quote_overflow');
  });

  it('refuses a cost transition that would drive a held bucket below zero', () => {
    const ledger = {
      capMicroUsd: 100,
      actualMicroUsd: 0,
      reservedMicroUsd: 10,
    ...
    let thrown: unknown;
    try {
      applyNodeSlideBudgetCostDelta(ledger, {
        actualMicroUsd: 0,
        reservedMicroUsd: -11,
        unreconciledMicroUsd: 0,
      });
    ...
    expect((thrown as NodeSlideBudgetLedgerError).code).toBe('quote_overflow');
  });
  ...
  it('rejects a persisted state whose durable digest no longer matches its body', () => {
    const state = durableState();
    const tampered: NodeSlideBudgetLedgerState = { ...state, actualMicroUsd: 1 };
    let thrown: unknown;
    try {
      assertNodeSlideBudgetLedgerState(tampered);
    } catch (error) {
      thrown = error;
    }
    expect(thrown).toBeInstanceOf(NodeSlideBudgetLedgerError);
    expect((thrown as NodeSlideBudgetLedgerError).code).toBe('invalid_budget_ledger');
    expect((thrown as NodeSlideBudgetLedgerError).message).toContain('digest mismatch');
  });
  ...
  it('chains event digests to the canonical budget core', () => {
    ...
    expect(nodeSlideBudgetEventDigest({ ...event, sequence: 2 })).not.toBe(
      nodeSlideBudgetEventDigest(event),
    );
  });
});
