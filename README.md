...
| Local upgrade fixture → clean-checkout package proof | The historical `0.0.1` fixture worked locally only because its `dist/` entrypoint existed under a repository-wide ignore rule; a clean Linux runner correctly rejected the tarball. | Track the fixture's exact runtime and declaration entrypoints explicitly, keep the packed-entrypoint assertion fail-closed, and run the portability proof in the downstream clean-checkout job before staging anything into NodeRoom. |
| Shared tarball directory → scoped artifact lanes | NodeRoom's strict NodeSlide consumer correctly rejected a valid Gym tarball when CI placed both package families in one directory. | Pack `@nodekit/gym-core` into its own candidate directory, leave `NODESLIDE_PACKAGE_ARTIFACT` limited to versioned `@nodeslide/*` tarballs, and statically test that the two lanes cannot collapse again. |
...
2. **Capability-honest labels — done** — the Design inspector says “Import design style from PPTX,” states that content is not imported, and labels the semantic NodeSlide → static-image PPTX math boundary.
