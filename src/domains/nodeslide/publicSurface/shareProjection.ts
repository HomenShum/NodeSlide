...
function publicSlide(slide: Slide): Slide {
  return {
    ...
    // `archetype` decides how the ArtifactSpec compiler groups a slide's marks.
    // Dropping it would make the public page compile under different rules than
    // the export the owner validated, so it is carried; it is not deck content.
...
function publicDeckPage(
  ...
  const outline = projected
    ...
    .join('');
  // `renderSlideHtml` runs `assertNodeSlideArtifactCompilation` and throws when
  // the snapshot does not compile. That gate is deliberate and is not caught
  // here: `buildShareProjection` turns it into an honest refusal page.
...
export async function buildShareProjection(
  ...
  try {
    ...
    // `renderSlideHtml` fails closed when the published snapshot does not
    // compile against the ArtifactSpec contract. A half-rendered page would be
    // a worse answer than a refusal, and the compilation detail names internal
    // element ids, so it stays in the deployment log.
    return {
      status: 500,
      headers: HTML_HEADERS,
      body: refusalPage(
        'This presentation cannot be rendered',
        'The published snapshot did not pass the rendering contract, so this page refuses to show a partial deck. The interactive presenter may still open it.',
        app,
      ),
    };
  }
