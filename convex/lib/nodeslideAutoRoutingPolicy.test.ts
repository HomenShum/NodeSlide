...
// Parity's fixture used 'openai/gpt-5.6-luna', which is NOT in this repo's
// NODESLIDE_AGENT_MODELS catalog. Widening the model union is a product
// decision, not a merge one, so the fixture is retargeted at the OpenAI-family
// route this catalog does carry. Structural intent is preserved: a third
// premium OpenRouter route that sorts after z-ai/glm-5.2 and claude-sonnet-5,
// which is what the ordering and fallback assertions below actually depend on.
const GPT_ALT = {
  provider: 'openrouter',
  catalogModelId: 'openai/gpt-5.6-sol',
} as const satisfies NodeSlideRouteReference;
...
function selected(decision: NodeSlideAutoRoutingDecision) {
...
function refused(decision: NodeSlideAutoRoutingDecision) {
...
describe('NodeSlide fail-closed auto routing policy', () => {
    ...
    it.each([
      {
        ...
        consent: consentFor(OPENROUTER_GLM, GPT_ALT),
        live: [availability(OPENROUTER_GLM, false), availability(GPT_ALT)],
      },
      {
        name: 'reversed consent and availability order',
        consent: {
          providers: ['openrouter' as const],
          models: [GPT_ALT.catalogModelId, OPENROUTER_GLM.catalogModelId],
        },
        live: [availability(GPT_ALT), availability(OPENROUTER_GLM, false)],
      },
    ])(
    ...
    it.each([
      ...
      ({ routes, live, expectedModel, expectedIndex, expectedOutcomes }) => {
        const decision = selected(
          decideNodeSlideAutoRoute(
            input({
              consent: consentFor(OPENROUTER_GLM, CLAUDE_SONNET, GPT_ALT),
              availability: live,
  ...
  describe('stable capability-aware selection', () => {
    it.each([
      ...
      const routes = [OPENROUTER_GLM, CLAUDE_SONNET, GPT_ALT] as const;
