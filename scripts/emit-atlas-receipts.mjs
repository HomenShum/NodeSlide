...
const MAX_PREFERENCE_BYTES = 1_048_576;
const MAX_PREFERENCE_ENTRIES = 10_000;
 ...
 * downstream reflects.
 */
export function preferenceIneligibility(receipt) {
 ...
 */
export function preferenceShapeProblems(parsed, label) {
  const problems = [];
  ...
  let collectionBoundExceeded = false;
  for (const [field, entries] of [
    ['preferred', parsed.preferred],
    ['notPreferred', parsed.notPreferred],
    ['notJudged', notJudged],
    ['evidence', evidence],
    ['renderBindings', parsed.renderBindings],
  ]) {
    if (Array.isArray(entries) && entries.length > MAX_PREFERENCE_ENTRIES) {
      say(`has ${entries.length} ${field} entries; the bound is ${MAX_PREFERENCE_ENTRIES}.`);
      collectionBoundExceeded = true;
    }
  }
  if (
    !isStringArray(parsed.preferred) ||
    !isStringArray(parsed.notPreferred) ||
    !evidence ||
 ...
 */
export function preferenceRunProblems(parsed, receipts, label) {
  const problems = [];
 ...
 */
export async function loadHumanPreference(declaredPath) {
  ...
  try {
    ...
    if (metadata.size > MAX_PREFERENCE_BYTES) {
      throw new Error(`file is ${metadata.size} bytes; the bound is ${MAX_PREFERENCE_BYTES} bytes`);
    }
...
export function preferenceFlag(argv) {
