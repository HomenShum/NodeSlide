...
describe('NodeSlide benchmark automation gate', () => {
  ...
  it('keeps the scheduled producer fixed to C01, E01, A05 and fail-closed visual judging', async () => {
    ...
    // nodeslide ships on npm (package-lock.json, `cache: npm` in CI); parity used pnpm.
    // The assertion is that the scheduled job invokes the producer and the judge — the
    // package manager is incidental and must match this repo, not the one it came from.
    expect(workflow).toContain('npm run nodeslide:bench:produce-live');
    expect(workflow).toContain('npm run nodeslide:bench:taste-judge');
