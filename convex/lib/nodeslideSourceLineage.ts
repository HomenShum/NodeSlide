...
function sourceIdsForClaimOperation(operation: PatchOperation): string[] {
  ...
  if (operation.op === 'update_chart') {
    // This repo's `update_chart` also carries partial `chartType`/`series` updates that keep the
    // element's existing chart data and provenance. Only a full `chart` replacement can rebind a
    // source, so a partial update contributes no source ID.
  ...
  const claim =
    operation.op === 'replace_text'
      ...
      : operation.op === 'update_chart'
        ? {
            operation: operation.op,
            chart: operation.chart ?? null,
            // Partial updates change the rendered claim without touching `chart`; folding them in
            // keeps the digest a real fingerprint of the claim rather than a constant.
            chartType: operation.chartType ?? null,
            series: operation.series ?? null,
          }
        : {
            operation: operation.op,
            element: {
