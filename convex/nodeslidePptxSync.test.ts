...
describe('NodeSlide linked PPTX server runtime', () => {
  ...
  // Parity asserted this against a hand-written table list in
  // convex/lib/nodeslideDeckDeletion.ts. This repo has no such list: the
  // erasure contract is derived from convex/schema.ts at load time. Reading the
  // derived contract is the same claim against the mechanism that is actually
  // in force here — and a stronger one, because it also pins the scoping field
  // and the index the deletion will scan by.
  it('registers the link table for schema persistence and atomic deck erasure', () => {
    ...
    const entry = NODESLIDE_ERASURE_CONTRACT.find(
      (candidate) => candidate.table === 'nodeslide_pptx_sync_links',
    );
    expect(entry, 'the link table must be walked by the whole-deck erasure').toBeDefined();
    expect(entry?.scope).toEqual({ kind: 'deckScoped', field: 'deckId', index: 'by_deck' });
  });
});
