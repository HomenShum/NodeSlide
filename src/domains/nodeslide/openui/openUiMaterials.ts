/**
 * OpenUI Phase 0 — a bounded lab that compiles ONE deterministic visual material into ONE
 * unapplied add-slide proposal.
 *
 * NAMING, and why it is not `VisualMaterial*`.
 *
 * This repo already owns a `VisualMaterial` family — `NodeSlideVisualMaterial`,
 * `NodeSlideVisualMaterialKind`, `NodeSlideVisualMaterialStatus`,
 * `NodeSlideVisualMaterialInventory` in `convex/lib/nodeslideStoryContext.ts`, read by
 * `convex/lib/nodeslideDesignPlan.ts`. That family answers "what evidence do we have available to
 * build a deck FROM" — an inventory of chart/table/photo material with an availability status.
 *
 * The family below answers a different question: "what does one already-decided slide LOOK like",
 * as a renderable spec. Both vocabularies use the words material, kind, and provenance, and both
 * would sit in the same import graph. Two families that share nouns but not meaning is how a
 * reader ends up passing one where the other is expected and getting a type error that explains
 * nothing — or worse, not getting one. So the incoming family carries an `OpenUi` prefix, which
 * also names the thing that is actually true about it: it exists to feed the OpenUI Lang renderer.
 *
 * The renames are recorded, one row each with this reason, in the RENAMES table of
 * parity-studio's `scripts/port-audit.mjs`.
 */
...
/**
 * The identifier is prefixed; the literal is not. The string is a wire value that already exists
 * in parity-recorded specs, and renaming an identifier is a refactor while renaming a schema
 * version is a contract break. Only the first was asked for.
 */
export const NODESLIDE_OPENUI_MATERIAL_VERSION = 'nodeslide.visual-material/v1' as const;

export interface OpenUiMaterialClaim {
  label: string;
  value: string;
  unit: string;
  context: string;
...
export interface OpenUiMaterialSpec {
  schemaVersion: typeof NODESLIDE_OPENUI_MATERIAL_VERSION;
  id: string;
  kind: 'transformation_ladder' | 'chart';
  eyebrow: string;
  title: string;
  subtitle: string;
  claims: readonly OpenUiMaterialClaim[];
  provenanceLabel: string;
  verification: 'unverified_scenario' | 'source_bound';
}

export interface OpenUiMaterialValidation {
  ok: boolean;
  issues: string[];
...
/**
 * The one shipped fixture, and it is a fixture.
 *
 * `verification: 'unverified_scenario'` is not a placeholder waiting to be promoted. The numbers
 * came from a user-supplied scenario brief, nobody bound them to a source, and the surface that
 * renders this spec renders that fact next to it (see `OpenUiMaterialWorkbench`, and the
 * `data-verification` attribute it publishes). `validateOpenUiMaterialSpec` refuses any spec that
 * claims `unverified_scenario` while hiding the word "unverified" from its own provenance label,
 * so the honesty is enforced by the validator rather than by whoever edits this constant next.
 */
export const AI2027_TRANSFORMATION_LADDER: OpenUiMaterialSpec = {
  schemaVersion: NODESLIDE_OPENUI_MATERIAL_VERSION,
...
export function validateOpenUiMaterialSpec(spec: OpenUiMaterialSpec): OpenUiMaterialValidation {
  const issues: string[] = [];
...
export function compileOpenUiMaterialProposal(
  spec: OpenUiMaterialSpec,
  deck: Deck,
  activeSlide: Slide,
