...
import type { DeckPatch, DeckSnapshot, SlideElement } from '../../../../shared/nodeslide';
...
import { exportNodeSlideJson, parseNodeSlideJson } from './jsonSpec';
...
describe('selected-element JSON editing', () => {
  ...
  // `latestDeckPatch` is not the same as the shipped `patches.at(-1)`: it orders
  // by updatedAt and breaks ties on id, so it is correct for an unsorted list.
  it('picks the newest patch by updatedAt rather than by array position', () => {
    const patch = (id: string, updatedAt: number) => ({ id, updatedAt }) as unknown as DeckPatch;
    const unsorted = [patch('patch:b', 300), patch('patch:c', 100), patch('patch:a', 200)];

    expect(latestDeckPatch(unsorted)?.id).toBe('patch:b');
    expect(unsorted.at(-1)?.id).toBe('patch:a');
    expect(latestDeckPatch([patch('patch:a', 500), patch('patch:b', 500)])?.id).toBe('patch:b');
    expect(latestDeckPatch([])).toBeNull();
  });
  ...
  // The shipped `downloadDeckJson` writes a bare snapshot; `parseNodeSlideJson`
  // only accepts the versioned envelope. Porting jsonSpec did NOT change the
  // download wire format, so exported files are not re-importable yet. This
  // pins that seam so it cannot close silently in either direction: whoever
  // flips the download to the envelope must flip these two assertions with it.
  it('writes the shipped bare-snapshot wire format, which the envelope parser rejects', async () => {
    ...
    try {
      ...
      expect(JSON.parse(downloadedText)).toEqual(snapshot);

      // Not yet round-trippable: the shipped file has no format/version header.
      const reparsed = parseNodeSlideJson(downloadedText);
      expect(reparsed.ok).toBe(false);
      // The envelope form is what the parser does accept.
      const exported = exportNodeSlideJson(snapshot);
      expect(exported.ok).toBe(true);
      if (!exported.ok) throw new Error('Expected the snapshot to export.');
      expect(parseNodeSlideJson(exported.json).ok).toBe(true);
