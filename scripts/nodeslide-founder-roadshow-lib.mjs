...
export const ROADSHOW_ATTACHMENT_GROUPS = Object.freeze([
  Object.freeze({
    ...
    // Re-pointed on the port: this repo keeps the product definition at docs/PRD.md and
    // docs/TDD.md. The originating repo's docs/submission/nodeslide-{prd,tdd}.md do not
    // exist here, and `resolveRoadshowInputPaths` only joins strings — it never stats them —
    // so the stale paths produced a green plan for two files that were not on disk.
    // `storyboard inputs all exist on disk` in the sibling test is the sensor for that.
    sources: Object.freeze(['docs/PRD.md', 'docs/TDD.md']),
  }),
  Object.freeze({
...
/**
 * Anchor for the multi-slide write-scope chip.
 *
 * NOT LIVE IN THIS REPO. The shipped write-scope row is Deck / This slide / Selection
 * (`ScopeChoice` in src/domains/nodeslide/inspector/AiInspector.tsx); there is no
 * "Selected slides (N)" chip for this regex to match. The pattern is kept because it is a
 * correct description of the affordance and the scene that needs it is declared
 * `pending-product-selector` in the storyboard, which makes `assertRoadshowLiveReady` refuse
 * to record — a named refusal instead of a selector timeout that reads like flake.
 */
