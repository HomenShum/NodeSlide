...
/**
 * Sibling Convex modules this file dispatches to by name, and whether they exist
 * in this repo yet.
 *
 * `internal` in `convex/_generated/api.js` is `anyApi` — a proxy that hands back
 * a plausible-looking function reference for *any* path, present or not. So a
 * missing sibling module is not a `TypeError` at the call site and not a
 * compile error either; it is a dispatch failure that arrives after the job row
 * has already been written and the caller has already been told "queued". The
...
/**
 * Fail closed at the front of a start mutation. Called before the idempotency
 * lookup so a refused start cannot leave a reserved key behind either.
 */
function requireJobOrchestrator(): void {
  if (NODESLIDE_ABSENT_JOB_SIBLINGS.length === 0) return;
  throw new Error(
    `NodeSlide durable jobs are unavailable: the orchestrator module${NODESLIDE_ABSENT_JOB_SIBLINGS.length === 1 ? '' : 's'} ${NODESLIDE_ABSENT_JOB_SIBLINGS.join(', ')} ${NODESLIDE_ABSENT_JOB_SIBLINGS.length === 1 ? 'is' : 'are'} not deployed in this repo. No job was created. Use the synchronous nodeslideAgent actions until the workflow modules land.`,
  );
}
