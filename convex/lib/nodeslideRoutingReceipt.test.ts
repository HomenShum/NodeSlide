...
describe('buildNodeSlideCreateRoutingReceipt', () => {
  it('selects the requested external route with a cost estimate when recently proven available', () => {
    const receipt = buildNodeSlideCreateRoutingReceipt({
      ...
      // This repo's catalog attests ['high','xhigh'] for z-ai/glm-5.2; parity's
      // also attested 'medium'. The policy fail-closes on an unattested effort
      // rather than downgrading it, so 'medium' here refused instead of
      // selecting. 'high' is the equivalent attested effort in THIS catalog.
      providerEffort: 'high',
  ...
  it('refuses honestly when the requested route has no in-window availability signal', () => {
    const receipt = buildNodeSlideCreateRoutingReceipt({
      providerMode: 'openrouter_free',
      providerModel: 'z-ai/glm-5.2',
      // Must be an effort THIS catalog attests, otherwise the route is refused
      // for reasoning_effort_unsupported and this test passes for the wrong
      // reason — it would go green without ever exercising availability.
      providerEffort: 'high',
      briefCharacters: 800,
      attachmentCharacters: 0,
      signals: [],
    ...
    // Assert the REASON, not just that it refused. `code` being merely truthy
    // cannot tell an availability refusal apart from an effort refusal.
    expect(receipt.decision.code).toBe('route_unavailable');
