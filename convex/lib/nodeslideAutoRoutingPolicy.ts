...
/**
 * NodeSlide auto-routing policy — `nodeslide.auto-routing/v1`.
 *
 * Provider/model selection gated on consent, capability, context window, live
 * availability and a per-request cost cap. Consumed by nodeslideRoutingReceipt.ts.
 *
 * This is a SIBLING of `nodeslideRoutingPolicy.ts`, not a version of it. That
 * module hosts `nodeslide.model-routing/v1` — the orchestrator/worker copy split
 * consumed by nodeslideEditPlanner.ts. Two policies, two decisions, neither
 * superseding the other.
 *
 * They are deliberately separate FILES because this module builds its catalog at
 * module scope from the pi-ai OpenRouter catalog and `shared/nodeslide`, whereas
 * `nodeslideRoutingPolicy.ts` is import-free by design so it can be lifted into a
 * product-neutral `model-routing.v1` contract unchanged. Merging the two into one
 * file would make every importer of `resolveNodeSlideRouting` — including shipped
 * nodeslideEditPlanner.ts — pay this module's load cost for a capability it does
 * not use. Do not merge them back.
 *
 * The `NODESLIDE_AUTO_ROUTING_*` / `NodeSlideAutoRouting*` names are namespaced so
 * the unprefixed `NODESLIDE_ROUTING_POLICY_VERSION` and `NodeSlideRoutingDecision`
 * keep meaning policy 1's, which nodeslideEditPlanner.ts writes into a shipped receipt.
 */
