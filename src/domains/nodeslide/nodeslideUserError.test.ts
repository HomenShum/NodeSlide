...
describe('NodeSlide user-facing errors', () => {
  ...
  /**
   * Wiring guard. Every toast the studio raises runs through its local `errorMessage`
   * helper; if that helper stops delegating to the sanitizer, raw Convex transport noise
   * ("[CONVEX M(...)] Server Error \n at handler ...") reaches the user again. A unit test
   * on the sanitizer alone cannot see that regression, so assert the call site.
   */
