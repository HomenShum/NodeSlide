...
/**
 * Two of these tests shell out to the real npm CLI to pack and install a fixture package into a
 * temp sandbox. That work genuinely costs ~3.5s on this machine, against vitest's 5000ms default —
 * about 1.4s of headroom, which disappears once the full suite runs them alongside everything else.
 * The result was a test that passed when run alone and failed 3 for 3 under `vitest run`, which
 * reads as flakiness but is really a budget set for unit-test work being applied to a package
 * install.
 *
 * The budget is per-test rather than global on purpose: raising testTimeout for the whole suite
 * would hide genuinely hung tests elsewhere. Only the tests that pay real npm cost get real time.
 */
const REAL_NPM_INSTALL_TIMEOUT_MS = 30_000;
...
describe('immutable install and upgrade consumer gates', () => {
  ...
  it(
    'typechecks and executes an isolated installed package',
    async () => {
    ...
    REAL_NPM_INSTALL_TIMEOUT_MS,
  );

  it(
    'rejects a valid npm tarball whose declaration passes but runtime entry is broken',
    async () => {
