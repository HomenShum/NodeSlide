/**
 * Deck-scoped agent memory.
 *
 * Flat-model rewrite of parity-studio's `convex/nodeslideScopedMemory.ts`
 * (DECOUPLING_PLAN §8 D1). Parity partitioned memory across
 * workspace > project > deck. NodeSlide has neither workspaces nor projects,
 * so the hierarchy is re-rooted at the deck: `deck` > `session`. Both levels
 * are derived from the authoritative deck row (`id`, `clientSessionId`) and
 * never from caller input, which is the invariant that made parity's version
 * safe and is the reason a `run` scope is NOT included — a runId is caller
 * input, and nothing needs it yet.
 *
 * The scope keys are the ones `nodeSlideMemoryScopeKey` already canonicalizes
 * for this repo, so a grant's `agentPolicy.scopes.memoryScopeKeys` gates
 * memory access with no new vocabulary.
 *
 * `mergeNodeSlideScopedAndLegacyMemories` bridges to the pre-existing
 * deck-keyed `nodeslide_agent_memories` table, so scoped memory is additive
...
export type NodeSlideScopedMemoryScopeKind = 'deck' | 'session';
...
/** A deck row is the only authority for scope identifiers. */
type ScopeSourceDeck = {
  readonly id: string;
  readonly clientSessionId: string;
};

type AuthorizedDeck = Pick<Doc<'nodeslide_decks'>, 'id' | 'clientSessionId' | 'ownerAccessKey'>;

const scopeKindValidator = v.union(v.literal('deck'), v.literal('session'));
...
/**
 * Creates one scoped memory. The capability (owner key or grant token) is
 * consumed only by the access gate and is never copied into a row or response.
 */
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    const deck = await authorizeScopedMemory(ctx, args, 'write', args.scopeKind);
...
export const retrieve = query({
  ...
  handler: async (ctx, args) => {
    const deck = await authorizeScopedMemory(ctx, args, 'read');
    const scopes = nodeSlideScopedMemoryScopes(deck);
    return selectNodeSlideScopedMemories(await readScopeRows(ctx, scopes), scopes, args.limit);
  },
});
...
async function readScopeRows(
  ctx: ReadCtx,
  scopes: readonly NodeSlideScopedMemoryScope[],
): Promise<NodeSlideScopedMemoryRecord[]> {
  return (
    await Promise.all(
      scopes.map(
        async (scope) =>
...
/**
 * Two capabilities reach scoped memory: the deck owner key, and a deck grant
 * token whose nested agent policy names both the memory capability and every
 * scope key being touched. Whichever is used, the scope identifiers come from
 * the deck row, so a caller cannot name a scope they were not granted.
 */
async function authorizeScopedMemory(
  ...
  if (args.ownerAccessKey !== undefined) {
    return await requireOwnerAccess(ctx, args.deckId, args.ownerAccessKey);
  }
  const deck = await findDeckRow(ctx, args.deckId);
  if (!deck || args.token === undefined || !GRANT_TOKEN_PATTERN.test(args.token)) {
    throw new Error(SCOPE_ACCESS_DENIED);
  }
  const grant = await ctx.db
    .query('nodeslide_deck_grants')
    .withIndex('by_token_digest', (index) =>
      index.eq('tokenDigest', nodeSlideDeckGrantTokenDigest(args.token as string)),
    )
    .unique();
  if (
    !grant ||
    grant.deckId !== deck.id ||
    grant.revokedAt !== undefined ||
    Date.now() >= grant.expiresAt ||
    !isNodeSlideDeckAccessPolicy(grant.policy) ||
    grant.policy.deckId !== grant.deckId ||
    grant.policy.role !== grant.role
  ) {
    throw new Error(SCOPE_ACCESS_DENIED);
  }
  const deckDecision = evaluateNodeSlideDeckAccess(grant.policy, {
    deckId: deck.id,
...
export function nodeSlideScopedMemoryScopes(deck: ScopeSourceDeck): NodeSlideScopedMemoryScope[] {
  return [nodeSlideScopedMemoryScope(deck, 'deck'), nodeSlideScopedMemoryScope(deck, 'session')];
}

export function nodeSlideScopedMemoryScope(
  deck: ScopeSourceDeck,
  kind: NodeSlideScopedMemoryScopeKind,
): NodeSlideScopedMemoryScope {
  const deckId = boundedIdentifier(deck.id, 'deckId');
  if (kind === 'deck') {
    return { kind, scopeKey: nodeSlideMemoryScopeKey({ kind, deckId }), deckId };
  }
  const sessionId = boundedIdentifier(deck.clientSessionId, 'sessionId');
  return {
    kind,
    scopeKey: nodeSlideMemoryScopeKey({ kind, deckId, sessionId }),
    deckId,
    sessionId,
  };
}
...
export function createScopedMemoryRecord(args: {
  ...
  return {
    ...
    deckId: args.scope.deckId,
    ...(args.scope.sessionId ? { sessionId: args.scope.sessionId } : {}),
    category: args.category,
    content,
    contentDigest,
    bindingDigest,
...
/**
 * Pure compatibility bridge for the pre-existing deck-only memory table
 * (`nodeslide_agent_memories`). Session-level rows outrank deck-level rows;
 * scoped rows win same-rank digest collisions. Legacy rows can only join the
 * exact deck scope and remain subject to the same item and byte ceilings.
 */
...
function isExactScopedMemoryRecord(
  ...
  return (
    ...
    row.sessionId === expectedScope.sessionId &&
    row.id === expectedId &&
...
/** Narrowest scope wins: a session memory outranks a deck-wide one. */
function scopeRank(kind: NodeSlideScopedMemoryScopeKind): number {
  return kind === 'session' ? 2 : 1;
}
