...
/**
 * A typed visual bound to a real evidence source. Two of these are not
 * decoration: `assertNodeSlideArtifactCompilation` refuses any deck of six or
 * more slides that carries meaningful typed visuals on fewer than two of them,
 * and a chart artifact with no source is refused outright. A fixture without
 * them is not a publishable deck, so it cannot stand in for one here.
 */
function chartElement(slideId: string, id: string): SlideElement {
  return {
    id,
    slideId,
    name: `Coverage chart on ${slideId}`,
    kind: 'chart',
    bbox: { x: 0.06, y: 0.52, width: 0.62, height: 0.36 },
    rotation: 0,
    chart: {
      chartType: 'bar',
      labels: ['Q1', 'Q2', 'Q3'],
      series: [{ name: 'Qualified pipeline', values: [41, 58, 72] }],
      unit: '$M',
...
function publishedDeck(
  ...
  const elements = slides.flatMap((slide, index) => [
    ...
    ...(index < 2 ? [chartElement(slide.id, `element:${index + 1}:chart`)] : []),
  ]);
...
describe('the projection stays bounded and fails closed', () => {
  ...
  it('refuses with 500 when the snapshot does not pass the ArtifactSpec contract', async () => {
    ...
    // A six-slide deck with no typed visual is exactly what that gate refuses;
    // the projection must turn the throw into a refusal, never a partial page.
    const published = publishedDeck({ slideCount: 8 });
    const textOnly: PublishedNodeSlide = {
      ...published,
      snapshot: {
        ...published.snapshot,
        slides: published.snapshot.slides.map((slide) => ({
          ...slide,
          elementOrder: slide.elementOrder.filter((id) => !id.endsWith(':chart')),
        })),
        elements: published.snapshot.elements.filter((element) => element.kind !== 'chart'),
      },
    };
    ...
    const projection = await buildShareProjection({
      url: `/s/${SHARE_SLUG}`,
      origin: ORIGIN,
      loadPublishedDeck: async () => textOnly,
    });

    expect(projection.status).toBe(500);
    expect(projection.body).toContain('This presentation cannot be rendered');
    expect(projection.body).not.toContain('data-slide-id');
    // The compilation detail names internal element ids; it stays in the log.
    expect(projection.body).not.toContain('artifact_visual_coverage');
  });
