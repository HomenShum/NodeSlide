...
function createRoundTripSnapshot(): DeckSnapshot {
  ...
  snapshot.elements = [
    ...
    },
    {
      ...
      // Bound to the real source seeded below, not to a placeholder id.
      //
      // This fixture arrived from parity-studio with `sourceIds: []` and the export failed here
      // with artifact_claim_evidence_binding — "chart artifact must retain at least one canonical
      // evidence source" (shared/nodeslideArtifactSpec.ts:549). That is this repository being
      // STRICTER than the one the file came from, and the gate is right: a chart with no evidence
      // is the thing the evidence layer exists to refuse.
      //
      // The gate only tests `sourceIds.length === 0`, so a made-up id would have passed it. That
      // would satisfy the check while breaking the rule it encodes, which is worse than the
      // failure.
      sourceIds: ['source:roundtrip:revenue'],
      locked: false,
      exportCapabilities: ['web_native', 'pptx_editable', 'google_importable'],
      version: 1,
    },
  ];
  // The scaffold seeds `sources: []`. The chart above cites this record, so the citation resolves
  // to something a reviewer could actually open rather than to a dangling id.
  snapshot.sources.push({
    id: 'source:roundtrip:revenue',
    deckId: snapshot.deck.id,
    title: 'Quarterly revenue, fixture data',
    sourceType: 'internal',
    retrievedAt: 3,
    citation: 'Fixture data for the PPTX round-trip test. Not a real revenue figure.',
  });
